...
# Fornisci dati simulati per NASDAQ per il periodo 2025-2027
def get_nasdaq_data():
    # Poiché stiamo proiettando nel futuro, creiamo dati simulati
    # Partiamo dall'attuale valore del NASDAQ
    try:
        ...
        nasdaq_recent = yf.download(
            "^IXIC", start=start_date, end=end_date, interval="1d"
        )
        last_price = float(nasdaq_recent["Close"].iloc[-1])  # Converti in float
    except:
        # In caso di errore, utilizziamo un valore fisso
        last_price = 16000.0  # Valore approssimativo del NASDAQ
    ...
    # Assumiamo una crescita annuale media del 10% con volatilità maggiore rispetto a S&P500
    annual_growth = 0.10
    ...
    # Volatilità settimanale (maggiore rispetto a S&P500)
    weekly_volatility = 0.03
    ...
    for i in range(weeks_in_projection):
        ...
        cycle1 = 0.015 * np.sin(2 * np.pi * i / 52)  # ~1 anno
        cycle2 = 0.03 * np.sin(2 * np.pi * i / (52 * 4))  # ~4 anni
        ...
        high_price = max(float(new_price), float(open_price)) * (
            1 + abs(np.random.normal(0, daily_volatility))
        )
        low_price = min(float(new_price), float(open_price)) * (
            1 - abs(np.random.normal(0, daily_volatility))
        )

        # Volume con trend e casualità
        base_volume = 5000000000  # Volume medio tipico per NASDAQ
...
def create_bradley_chart():
    ...
    ax1.set_title("BRADLEY SIDEROGRAPH 2025-2027 & NASDAQ", fontsize=18, weight="bold")
    ax1.set_xlabel("")
    ax1.set_ylabel("NASDAQ", fontsize=14)

    # Plot dei dati candlestick simulati
    for i in range(len(nasdaq_data)):
        date = nasdaq_data.index[i]
        open_price = float(nasdaq_data["Open"].iloc[i])
        close_price = float(nasdaq_data["Close"].iloc[i])
        high_price = float(nasdaq_data["High"].iloc[i])
        low_price = float(nasdaq_data["Low"].iloc[i])
        ...
        rect = Rectangle(
            (mdates.date2num(date), body_bottom),
            0.6,
            body_height,
            edgecolor=color,
            facecolor=color,
    ...
    ax1.plot(
        nasdaq_data.index, nasdaq_data["MA50"], color="red", linewidth=1.5, label="MA50"
    )
    ...
    ax2.bar(nasdaq_data.index, nasdaq_data["Volume"], color="darkblue", alpha=0.5)
    ...
    for i, event in enumerate(major_events):
        ...
        ax3.annotate(
            ...
            xytext=(0, y_offset * 100),  # Usa punti invece di frazioni
            textcoords="offset points",
            ha="center",
            va="center",
            fontsize=9,
    ...
    plt.savefig(
        "examples/siderograph/bradley_siderograph_nasdaq.png", bbox_inches="tight"
    )
    plt.savefig(
