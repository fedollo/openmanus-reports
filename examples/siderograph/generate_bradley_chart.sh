...
echo "Generazione grafico Bradley Siderograph con S&P500"
echo "=================================================="

# Istruzione per l'utente
echo "Questo script installerà le librerie Python necessarie e genererà il grafico."
echo "Per favore, attendi il completamento del processo."
echo ""

# Verifica l'installazione di pip
if ! command -v pip3 &> /dev/null; then
    echo "Error: pip3 non risulta installato. Per favore installa pip3."
    exit 1
fi

# Installa le dipendenze necessarie
echo "Installazione delle dipendenze..."
pip3 install yfinance pandas matplotlib mplfinance --user

# Verifica se l'installazione è andata a buon fine
if [ $? -ne 0 ]; then
    echo "Errore durante l'installazione delle dipendenze. Uscita..."
    exit 1
fi
...
# Crea una versione modificata dello script senza l'importazione problematica
SCRIPT_PATH="examples/siderograph/create_bradley_chart.py"
TEMP_SCRIPT="examples/siderograph/temp_bradley_chart.py"

echo "Creazione dello script temporaneo..."
cat > $TEMP_SCRIPT << 'EOF'
#!/usr/bin/env python3
...
# Rendi lo script temporaneo eseguibile
chmod +x $TEMP_SCRIPT

# Esegui lo script temporaneo
echo "Generazione del grafico in corso..."
python3 $TEMP_SCRIPT

# Verifica se l'esecuzione è andata a buon fine
if [ $? -ne 0 ]; then
    echo "Errore durante la generazione del grafico. Uscita..."
    exit 1
fi
...
echo "- examples/siderograph/bradley_siderograph_sp500.png"
echo "- examples/siderograph/bradley_siderograph_sp500.pdf"
echo ""
echo "Per visualizzare il PDF, puoi usare il comando:"
echo "open examples/siderograph/bradley_siderograph_sp500.pdf"
