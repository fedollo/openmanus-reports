...
def get_sp500_data():
    ...
    try:
        ...
        sp500_recent = yf.download(
            "^GSPC", start=start_date, end=end_date, interval="1d"
        )
        last_price = float(sp500_recent["Close"].iloc[-1])  # Converti in float
    except:
        # In caso di errore, utilizziamo un valore fisso
        last_price = 5000.0  # Valore approssimativo dell'S&P500
...
def create_bradley_chart():
    ...
    for i in range(len(sp500_data)):
        ...
        open_price = float(sp500_data["Open"].iloc[i])
        close_price = float(sp500_data["Close"].iloc[i])
        high_price = float(sp500_data["High"].iloc[i])
        low_price = float(sp500_data["Low"].iloc[i])
    ...
    ax1.plot(
        sp500_data.index, sp500_data["MA20"], color="blue", linewidth=1.5, label="MA20"
    )
    ax1.plot(
        sp500_data.index, sp500_data["MA50"], color="red", linewidth=1.5, label="MA50"
    )
    ...
    plt.savefig(
        "examples/siderograph/bradley_siderograph_sp500.png", bbox_inches="tight"
    )
    plt.savefig(
        "examples/siderograph/bradley_siderograph_sp500.pdf", bbox_inches="tight"
    )
